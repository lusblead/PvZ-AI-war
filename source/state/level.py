...
class Level(tool.State):
    ...
    def update(self, surface, current_time, mouse_pos, mouse_click):
        ...
        # 修改点：增加对 surface 是否存在的检查，支持 Headless 训练
        if surface:
            self.draw(surface)
    ...
    def addPlant(self):
        ...
        # 调用具体的植物生成逻辑
        self.createPlant(map_x, map_y, x, y, self.plant_name)
    ...
    # 修改点：新增方法，用于非 UI 方式种植植物
    def addPlantStatic(self, map_x, map_y, plant_name):
        if not self.map.isValid(map_x, map_y) or not self.map.isMovable(map_x, map_y):
            return False

        x, y = self.map.getMapGridPos(map_x, map_y)
        self.createPlant(map_x, map_y, x, y, plant_name)
        return True

    # 修改点：将植物生成逻辑抽离出来，供 addPlant 和 addPlantStatic 复用
    def createPlant(self, map_x, map_y, x, y, plant_name):
