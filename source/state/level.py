...
class Level(tool.State):
    ...
    def startup(self, current_time, persist):
        ...
        # 初始化鼠标/提示相关属性，避免在未进入选择/游戏前访问时抛出 AttributeError
        self.mouse_image = None
        self.mouse_rect = pg.Rect(0, 0, 0, 0)
        self.drag_plant = False
        self.hint_image = None
        self.hint_rect = pg.Rect(0, 0, 0, 0)
        self.hint_plant = False

        # 添加倒计时相关变量
        self.total_time = 300  # 300秒
    ...
    def update(self, surface, current_time, mouse_pos, mouse_click):
            ...
            # 更新倒计时
            if self.state == c.PLAY and not self.victory:
                time_diff = (current_time - self.last_time_update) / 1000
                if time_diff >= 1:
                    self.remaining_time -= int(time_diff)
                    self.last_time_update = current_time
                    if self.remaining_time <= 0:
                        self.remaining_time = 0
                        self.victory = True  # 达到300秒，游戏胜利
            ...
            if self.victory:
                # 设置下一状态为胜利画面，并标记本状态完成，交给 tool.py 的状态机去切换
    ...
    def draw(self, surface):
        # 绘制背景到 level 表面，然后把 level 放到屏幕
        self.level.blit(self.background, self.viewport, self.viewport)
        surface.blit(self.level, (0, 0), self.viewport)        # 根据状态绘制界面元素
        if self.state == c.CHOOSE:
            ...
            self.sun_group.draw(surface)        # 鼠标拖拽时在最上层绘制鼠标图 / 提示图
        if self.drag_plant:
            self.drawMouseShow(surface)        # 最后在最上层渲染倒计时，确保不会被后续 blit 覆盖
        if self.state == c.PLAY:
            font = pg.font.SysFont(None, 36)
            time_text = f"Time Remain: {self.remaining_time} S"
            text_surface = font.render(time_text, True, c.RED)
            surface.blit(text_surface, (10, c.SCREEN_HEIGHT - 40))